...
function print() {
  for (const dice of [1, 2, 3, 4, 5, 6]) { // Short enough not to use fancy range() implementation.
    ...
    for (const rollValue of frequencies.keys()) {
      if (rollValue < dice + 1) continue; // Ignore all least possible (100%) values
      ...
      console.log(`- ≥${rollValue}, ${toPercentage(probabilityAtLeast, 1)}% or ${numToOdds(probabilityAtLeast)}` +
                  (probabilityAtLeast !== probabilityAtLeastWithReroll
                       ? ` (rerolling? Then ${toPercentage(probabilityAtLeastWithReroll, 1)}% or ${
                             numToOdds(probabilityAtLeastWithReroll)})`
                       : ''))
    }
  }
}
